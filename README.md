...
- **Event Monitor**: tracks CPU spikes, memory usage, disk thresholds, and file system changes throughout the day and overnight
- **Logger**: writes every event to a SQLite database with timestamp, source, event type, severity, and description
- **REST API Layer**: a Flask application that exposes the event log and briefing data as queryable endpoints
- **Daily Briefing Generator**: aggregates all six data sources plus the overnight event log and outputs a formatted morning brief every morning on schedule
